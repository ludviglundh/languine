...
export async function init() {
    ...
    try {
        ...
        const sourceFile = path.join(process.cwd(), `${String(filesDir)}/${String(sourceLanguage)}.${String(fileFormat)}`);
        if (!(await fs
            .access(sourceFile)
            .then(() => true)
            .catch(() => false))) {
        ...
        for (const lang of targetLangs.map((l) => l.trim().toLowerCase())) {
            const targetFile = path.join(process.cwd(), `${String(filesDir)}/${lang}.${String(fileFormat)}`);
            if (!(await fs
                .access(targetFile)
                .then(() => true)
                .catch(() => false))) {
        ...
        await fs.writeFile(path.join(process.cwd(), "linguai.json"), JSON.stringify(config, null, 2));
